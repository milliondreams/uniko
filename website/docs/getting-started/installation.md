...
| `embed/hybrid` | Hybrid embed | same model as `embed/default` | Registered **only** when `EmbeddingConfig` sets `sparse_dimensions` or `multivector_dimensions` (e.g. the `bge-m3` preset). Writes the dense + ColBERT columns on `:Chunk` / `:Observation` in one forward pass — see [Hybrid retrieval](../guides/configuration.md#sparse-colbert-hybrid-retrieval). |
| `embed/sparse` | Sparse embed | same model as `embed/default` | Registered **only** when `EmbeddingConfig` sets `sparse_dimensions`. Backs the `sparse_embedding` column and, critically, the query-side vector for `uni.sparse.query` — a sparse query needs a `SparseEmbeddingModel`, which the `EmbedHybrid` alias cannot supply. |
| `embed/multivector` | Multi-vector embed | same model as `embed/default` | Registered **only** when `EmbeddingConfig` sets `multivector_dimensions`. Supplies the **query-side** per-token vector for ColBERT MaxSim; documents still get `colbert_embedding` from `embed/hybrid`. A ColBERT query needs a `MultiVectorEmbeddingModel`, which the `EmbedHybrid` alias cannot supply. |
