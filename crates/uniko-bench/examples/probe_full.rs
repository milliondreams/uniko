...
async fn main() -> anyhow::Result<()> {
    ...
    let queries = [
        ...
        (
            "Obs w/ triple",
            "MATCH (n:Observation) WHERE n.predicate IS NOT NULL RETURN count(n) AS n",
        ),
        (
            "Obs w/ temp",
            "MATCH (n:Observation) WHERE n.temporal_anchor IS NOT NULL RETURN count(n) AS n",
        ),
        ("Facts", "MATCH (n:Fact) RETURN count(n) AS n"),
        (
            "Facts w/ embed",
            "MATCH (n:Fact) WHERE n.embedding IS NOT NULL RETURN count(n) AS n",
        ),
        (
            "Cycles",
            "MATCH (n:ConsolidationCycle) RETURN count(n) AS n",
        ),
        (
            "HAS_CHUNK",
        ...
        (
            "OBSERVED_IN",
            "MATCH ()-[r:OBSERVED_IN]->() RETURN count(r) AS n",
        ),
        ("ABOUT", "MATCH ()-[r:ABOUT]->() RETURN count(r) AS n"),
        (
            "SUPPORTED_BY",
            "MATCH ()-[r:SUPPORTED_BY]->() RETURN count(r) AS n",
        ),
