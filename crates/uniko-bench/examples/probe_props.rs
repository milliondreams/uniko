...
async fn main() -> anyhow::Result<()> {
    ...
    for row in r.rows() {
        ...
        println!(
            "  vid={vid} labels={:?} props={:?}",
            n.labels,
            n.properties.keys().collect::<Vec<_>>()
        );
    }

    println!("\n== bench-agent Participant via RECORDED_BY edge ==");
    let r = session
        .query_with("MATCH (e)-[:RECORDED_BY]->(p) RETURN DISTINCT p AS p, id(p) AS vid LIMIT 3")
        .fetch_all()
        .await?;
    for row in r.rows() {
        let vid: i64 = row.get("vid").unwrap_or(-1);
    ...
    let r = session
        .query_with("MATCH (e)-[:RECORDED_BY]->(p) RETURN e AS e, id(e) AS vid LIMIT 3")
        .fetch_all()
        .await?;
    for row in r.rows() {
        let vid: i64 = row.get("vid").unwrap_or(-1);
