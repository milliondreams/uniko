//! Structured-NLP post-processing retained after the migration to xervo's
//! `NlpModel`.
//!
//! The tensor decoders (argmax/BIO/biaffine/SRL/CLS) moved into xervo; what
//! remains here operates on the already-decoded [`NlpResult`](super::types)
//! — predicate normalization, NER span construction (shared with the
//! [`adapter`](super::adapter)), and the dependency-tree → SVO / observation
//! extraction consumed by `observations`.

use super::types::{DepArc, NerEntityType, NerSpan};
...
pub(crate) fn parse_ner_type(suffix: &str) -> NerEntityType {
    match suffix {
