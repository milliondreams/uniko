...
async fn setup() -> Uni {
    ...
    db.schema()
        ...
        .done()
        .label("Owner")
        .property("oid", DataType::String)
        .done()
        .edge_type("OWNED_BY", &["Item"], &["Owner"])
        .done()
        .apply()
        .await
        .unwrap();
    ...
    tx.execute("MATCH (o:Owner {oid:'o1'}) CREATE (i:Item {tag:'a'})-[:OWNED_BY]->(o)")
        .await
        .unwrap();
    tx.execute("MATCH (o:Owner {oid:'o1'}) CREATE (i:Item {tag:'b'})-[:OWNED_BY]->(o)")
        .await
        .unwrap();
...
async fn first_tag(db: &Uni, program: &str, ret: &str) -> Option<Value> {
    ...
    r.rows()
        .and_then(|rows| rows.first().and_then(|row| row.get(ret).cloned()))
}
...
async fn single_node_key_without_fold() {
    let db = setup().await;
    db.rules()
        .register("CREATE RULE sn AS MATCH (i:Item) YIELD KEY i.tag AS tag")
        .await
        .unwrap();
    ...
    assert!(
        ...
        "KEY tag should be a String, got {v:?}"
    );
...
async fn multi_pattern_key_with_fold_works() {
    ...
    assert!(
        matches!(v, Some(Value::String(_))),
        "with FOLD, KEY tag should be a String, got {v:?}"
    );
    db.shutdown().await.unwrap();
}
