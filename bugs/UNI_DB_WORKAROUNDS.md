...
| RC12 | **Locy rule-by-name invocation BROKEN on 2.0** ❌ (verified 2026-06-03): `execute_rule("sequence_detector")` → `session.locy_with("sequence_detector")` fails `LocyParseError: expected locy_query` — 2.0 parses the arg as a Locy *query*, not a rule name. The Cypher fallback is **load-bearing** (removing it fails 3/4 `procedures_e2e`). | **should file** | **KEEP** `cortex/procedures.rs` Locy→Cypher fallback; `memory/rules/stdlib.rs:117` best-effort `create_rule`. Real fix = correct uniko's rule-invocation (locy query form) — affects all 4 stdlib rules routed through `execute_rule`. |
...
| Site | uni-db issue | Workaround | Evidence | Status |
|------|--------------|-----------|----------|--------|
| `src/procedures.rs` `fallback_sequence_query` | RC12: `execute_rule("sequence_detector")` **fails on 2.0** (`expected locy_query`) — **KEEP** | on `Locy` error, re-run as hand-written Cypher (`MATCH (e1)-[:FOLLOWED_BY]->(e2) … count(*) … WHERE n>=$thr`). Verified load-bearing: removing it fails 3/4 `procedures_e2e`. | comment | **should file** |
